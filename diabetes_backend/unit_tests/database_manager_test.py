...
from constants import DATETIME_FORMAT, DATABASE_TABLE
...
class TestDatabaseManager(unittest.TestCase):
    ...
    def test_get_last_record(self, mock_connect):
        ...
        mock_cur.execute.assert_called_with(
            sql.SQL("SELECT timestamp, id FROM {table} ORDER BY timestamp DESC LIMIT 1").format(
                table=sql.Identifier(DATABASE_TABLE),
            )
        )

    @mock.patch("psycopg2.connect")
    def test_save_glucose_data(self, mock_connect):
        test_data = [
